...
function _validateStartupDependencies() {
  var requiredGlobals = [
    'initI18N', 'applyDarkMode', 'AudioFX', 'FirebaseLeaderboard', 'NotificationService',
    'ProfilePill', 'PlayGamesService', 'initSettingsEvents', 'updateDailyCardUI',
    'initRankingFilters', 'renderRankingScreen', 'renderAchievementsScreen', 'showScreen'
  ];
  var missing = requiredGlobals.filter(function (k) { return typeof window[k] === 'undefined'; });
  if (missing.length) {
    console.error('[FlagMaster] Dependencias globales no encontradas:', missing.join(', '));
  }

  var expectedOrder = [
    'js/config.js', 'js/i18n.js', 'js/storage.js', 'js/utils.js', 'js/audio.js',
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  /* Listener de teclado unificado — gestiona tanto atajos de juego como
     la tecla Escape / botón Back de Android (Capacitor envía key='Escape'). */
...
function _handleAnswer(optionIndex) {
  ...
  else { updateHUD(state.score, state.streak, state.currentIndex, getTotalQuestions()); _autoAdvance = setTimeout(function () { _autoAdvance = null; _handleNext(); }, 1500); }
}
 ...
 *   1. Atajos de juego (teclas 1-4, Enter/Espacio para avanzar).
 *   2. Escape / botón Back de Android — cierra modales abiertos.
 *      En Capacitor el botón Back dispara un evento keydown con key='Escape',
 *      por lo que este handler es el punto único de cierre de todos los modales.
 */
function _handleKeyboard(e) {
  ...
  if ((e.key === 'Enter' || e.key === ' ') && state.answered) {
    e.preventDefault(); var nb = document.getElementById('btn-next'); if (nb && nb.style.display !== 'none') nb.click();
  }
}
